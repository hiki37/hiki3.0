...
# Ключевые слова для фильтра (без учёта регистра), общие для обоих источников.
# Пустой список [] = не фильтровать, брать всё.
KEYWORDS = [
    "сайт", "лендинг", "html", "css", "javascript", "js",
    "react", "vue", "next.js", "фронтенд", "верстка", "wordpress",
    "разработ", "программист", "верстальщик", "frontend", "backend", "фулстек",
]
...
def main():
    ...
    try:
        check_flru(seen)
        check_kwork_mail(seen)
        check_hh_crowd(seen)
        check_weblancer(seen)
    except Exception as e:
