    ...
    try:
        ...
        for num in data[0].split():
            ...
            project_links = re.findall(
                r'<a[^>]+href="(https?://(?:www\.)?kwork\.ru/[^"]*\d{3,}[^"]*)"[^>]*>(.*?)</a>',
                body, flags=re.IGNORECASE | re.DOTALL,
            )
            # Отсекаем то, что явно не карточка заказа (отписка, настройки и т.п.)
            SKIP_LINK_WORDS = ("unsubscribe", "settings", "notifications", "otpisat", "otpiska")
            project_links = [
                (link, strip_html(title)) for link, title in project_links
                if not any(w in link.lower() for w in SKIP_LINK_WORDS) and strip_html(title)
            ]

            if project_links:
                for link, title in project_links:
                    link_uid = "kwork-link:" + link
                    if link_uid in seen:
                        continue
                    seen.add(link_uid)
                    if matches_keywords(title):
                        notify("Kwork (почта)", title, "", link)
            else:
