...
def check_flru(seen):
    if not FLRU_ENABLED:
        return
    try:
        feed = fetch_feed(FLRU_RSS_URL)
    except Exception as e:
        print(f"[fl.ru] ошибка загрузки ленты ({type(e).__name__}): {e}")
        return
...
def check_kwork_mail(seen):
    ...
    try:
        ...
        for num in data[0].split():
            ...
            # Тема письма обычно общая ("Новые проекты на бирже Kwork - +3
            # подходящих") и никаких ключевых слов не содержит - весь смысл
            # в теле письма. А там сначала обычно идёт шапка/приветствие,
            # и сам список проектов может начинаться далеко не с первых
            # символов. Раньше матчинг смотрел только на первые 400 символов
            # и мог просто не долистать до реальных заголовков проектов -
            # теперь смотрим гораздо дальше, чтобы не терять подходящее.
            match_text = full_text[:5000]
            snippet = full_text[:400]  # а в само уведомление - покороче, для читаемости

            if matches_keywords(f"{subject} {match_text}"):
...
def check_superjob(seen):
    ...
    try:
        r = requests.get(
            ...
            params={"keyword": SUPERJOB_SEARCH_KEYWORD, "period": 7, "count": 100},
    ...
    if not seen:
        ...
        if FLRU_ENABLED:
            try:
                ...
                print(f"[fl.ru] ошибка индексации ({type(e).__name__}): {e}")
